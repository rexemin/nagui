...
app.layout = html.Div(children=[
    html.H1('Networks', className='m-4', id='header-graph'),
        ...
        dbc.Row([
            dbc.Col([
                ...
                dbc.Row([
                    ...
                    dbc.Col([
                        dbc.Input(id='restriction-graph', type='number', className='mx-1 my-1'),
                    ], width=3),
                ...
                dbc.Row([
                    ...
                    dbc.Col([
                        dbc.Input(id='cost-graph', type='number', className='mx-1 my-1'),
                    ], width=3),
                    dbc.Col([
            ...
            dbc.Col([
                dbc.Row([
                    dbc.Col([
                        html.H4('The network has 0 node(s) and 0 edge(s).', id='info-graph', className='mx-3'),
                    ], width=4),
                    dbc.Col([
...
@app.callback(
    ...
    [State(component_id='vertex-graph', component_property='value'),
     ...
     State(component_id='restriction-graph', component_property='value'),
     State(component_id='cost-graph', component_property='value'),
...
def update_graph(btn_vertex, btn_edge, btn_rm_v, btn_rm_e, btn_run, btn_reset, btn_empty, vertex_value, source, terminus,
    ...
    if btn_vertex is not None and btn_pressed == 0 and vertex_value != "":
        ...
        if not current_graph.has_node(input[0]):
            if len(input) == 1:
                current_graph.add_node(input[0], type='source', info='+ {}'.format(input[0]))
            elif len(input) == 2:
                flow = int(input[1])
                current_graph.add_node(input[0], type='source', flow=flow, info='+ {}, {}'.format(input[0], flow))
            elif len(input) >= 3:
                min_f = int(input[1])
                max_f = int(input[2])
                if min_f >= 0 and min_f <= max_f:
                    current_graph.add_node(input[0], type='source', min_flow=min_f, max_flow=max_f, info='+ {}, {}/{}'.format(input[0], min_f, max_f))
                else:
        ...
        elif not current_graph.has_node(source) and not current_graph.has_node(terminus):
...
def update_graph_info(graph):
    return "The network has {} node(s) and {} edge(s)".format(current_graph.number_of_nodes(), current_graph.number_of_edges())
...
@app.callback(
    Output(component_id='restriction-graph', component_property='value'),
    [Input(component_id='btn-edge-graph', component_property='n_clicks')]
)
def reset_weight_input(n_clicks):
    return 0
...
@app.callback(
    Output(component_id='cost-graph', component_property='value'),
    [Input(component_id='btn-edge-graph', component_property='n_clicks')]
)
def reset_weight_input(n_clicks):
    return 0
